...
public class SendStateMessage : IMessageCommand
{
    public string Command => "/stateMessage";
    
    public async Task<CommandResponse<string>> Handle(IMessageView messageView, long chatId, string? userMessage)
    {
        // Buttons
        ...
        // Send message!
        await messageView.SayWithMarkup("Как ты себя сейчас чувствуешь?", chatId, inline);
